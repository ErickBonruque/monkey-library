...
export default function EntradasPage() {
  ...
  const { data: entries, isLoading } = useQuery({ queryKey: ["entries"], queryFn: () => estoqueService.getEntries() })

  const { mutateAsync } = useMutation({
    mutationFn: (data: StockEntryFormData) => estoqueService.registerEntry(data),
    onSuccess: () => {
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  ...
                  <Input {...field} id={field.name} placeholder="Ex: Compra do fornecedor X" aria-invalid={fieldState.invalid} />
                  {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                </Field>
              )}
            />
